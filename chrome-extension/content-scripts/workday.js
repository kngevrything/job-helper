...
function parseWorkdayUrl(href) {
  ...
  // Path is typically /{locale}/{site}/details/{externalPath}, but some
  // tenants (e.g. Yahoo) insert an extra location segment before the
  // real slug. The slug is always the LAST path segment regardless, so
  // anchor "site" off "job"/"details" but take externalPath from the
  // end of the path, not a fixed offset from the anchor.
